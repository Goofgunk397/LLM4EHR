...
from transformers import AutoTokenizer, AutoModel, utils, AutoModelForSequenceClassification


def expand_and_init_token_embedding(new_token, new_token_string, tokeniser, encoder):
    new_token_tok = tokeniser(new_token_string, padding='longest')
    new_token_embed = encoder.token_string_embedding(torch.tensor(new_token_tok['input_ids']),
                                                     torch.tensor(new_token_tok['attention_mask']))['pooler_output']
...
class BioClinEncoder(nn.Module):
    ...
    def encode_ts(self, ts_in):
        ...
        ts_hidden = self.base_model.encoder(ts_out)['last_hidden_state']
    ...
    def forward(self, ts_in, seq_in, seq_attn_mask, seq_token_mask):
        ...
        ts_out_pool = self.ts_pooler(ts_out)
        ts_out_final = (ts_out, ts_out_pool)
        return seq_pool_out, ts_out_final, omega


class BioClinModel(nn.Module):
    def __init__(self, n_channel, max_steps=200, patch_size=5):
        super().__init__()
        self.encoder = BioClinEncoder(n_channel, max_steps, patch_size)
