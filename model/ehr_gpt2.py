...
def expand_and_init_token_embedding(new_token, new_token_string, tokeniser, encoder):
    ...
    for s in new_token_tok['input_ids']:
        s_embed = encoder.encoder.base_model.wte.weight.data[s]
...
class EHRGPTClinEncoder(nn.Module):
    """
    Accepts all cannonical gpt2 implementations, gpt2/gpt2_large/gpt2_XL
    """
    def __init__(self, n_channel, max_steps=200, patch_size=5, gpt_name='gpt2', freeze_model=True):
        super().__init__()
        self.base_model = AutoModel.from_pretrained(gpt_name)
    ...
    def set_token_embeddings(self, idx, new_embedding):
        self.base_model.wte.weight.data[idx] = new_embedding

    def fix_token_embeddings(self, new_token_idx=None):
        for param in self.base_model.wte.parameters():
            param.requires_grad = False

        if new_token_idx is not None:
            self.base_model.wte.weight.data[new_token_idx].requires_grad = True
    ...
    def forward(self, ts_in, seq_in, seq_attn_mask, seq_token_mask):
        ...
        seq_pool_out = torch.bmm(seq_token_mask,
                                 seq_out['last_hidden_state'])
...
class EHRGPTModel(nn.Module):
    def __init__(self, n_channel, max_steps=200, patch_size=5, gpt_name='gpt2', freeze_model=True):
        super().__init__()
        self.encoder = EHRGPTClinEncoder(n_channel, max_steps, patch_size, gpt_name, freeze_model=freeze_model)
